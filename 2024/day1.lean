...
def createFreqMap (l : List Nat) : Std.HashMap Nat Nat :=
  l.foldl (λ m n => m.insert n (m.getD n 0 + 1)) (Std.HashMap.emptyWithCapacity)
...
def left := List.mergeSort (nums.map (λ n => n[0]!)) (λ a b => a <= b)
def rght := List.mergeSort (nums.map (λ n => n[1]!)) (λ a b => a <= b)
def left_original := nums.map (λ n => n[0]!)
def right_original := nums.map (λ n => n[1]!)
